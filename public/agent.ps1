...
# RAM
$ram = @{ total_gb = 0; free_gb = 0; used_percent = 0; total_physical_gb = 0 }
try {
    $cs = Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue
    if ($cs -and $cs.TotalPhysicalMemory) {
        $ram.total_physical_gb = [math]::Round($cs.TotalPhysicalMemory / 1GB, 2)
    }
    ...
    if ($os) {
        ...
        $ram.total_gb = $totalGB
        $ram.free_gb = $freeGB
        $ram.used_percent = $usedPct
    }
} catch {}

# CPU
$processor = @{ name = "Unknown"; usage_percent = 0; cores = 0; threads = 0; max_clock_mhz = 0 }
try {
    $cpu = Get-CimInstance Win32_Processor -ErrorAction SilentlyContinue | Select-Object -First 1
    if ($cpu) {
        $processor.name = $cpu.Name.Trim()
        $processor.usage_percent = $cpu.LoadPercentage
        $processor.cores = $cpu.NumberOfCores
        $processor.threads = $cpu.NumberOfLogicalProcessors
        $processor.max_clock_mhz = $cpu.MaxClockSpeed
    }
} catch {}
...
# Extended msinfo32 Data
$systemInfo = @{ manufacturer = "Unknown"; system_sku = "Unknown" }
$osDetails = @{ caption = "Unknown"; build_number = "Unknown"; architecture = "Unknown"; install_date = "Unknown" }
$biosInfo = @{ vendor = "Unknown"; version = "Unknown"; boot_mode = "Unknown" }
$boardInfo = @{ manufacturer = "Unknown"; product = "Unknown" }

try {
    $cs = Get-CimInstance Win32_ComputerSystem -ErrorAction SilentlyContinue
    if ($cs.Manufacturer) { $systemInfo.manufacturer = $cs.Manufacturer.Trim() }
    if ($cs.SystemSKUNumber) { $systemInfo.system_sku = $cs.SystemSKUNumber.Trim() }
    
    $osWmi = Get-CimInstance Win32_OperatingSystem -ErrorAction SilentlyContinue
    if ($osWmi) {
        if ($osWmi.Caption) { $osDetails.caption = ([string]$osWmi.Caption -replace '^Microsoft\s+', '').Trim() }
        if ($osWmi.BuildNumber) { $osDetails.build_number = $osWmi.BuildNumber.Trim() }
        if ($osWmi.OSArchitecture) { $osDetails.architecture = $osWmi.OSArchitecture.Trim() }
        if ($osWmi.InstallDate) { $osDetails.install_date = Get-Date $osWmi.InstallDate -Format "yyyy-MM-dd" }
    }
    
    $biosWmi = Get-CimInstance Win32_BIOS -ErrorAction SilentlyContinue
    if ($biosWmi.Manufacturer) { $biosInfo.vendor = $biosWmi.Manufacturer.Trim() }
    if ($biosWmi.SMBIOSBIOSVersion) { $biosInfo.version = $biosWmi.SMBIOSBIOSVersion.Trim() }
    
    $firmware = Get-ItemProperty -Path "HKLM:\System\CurrentControlSet\Control" -Name "PEFirmwareType" -ErrorAction SilentlyContinue
    if ($env:firmware_type) { $biosInfo.boot_mode = $env:firmware_type }
    elseif ($firmware -and $firmware.PEFirmwareType -eq 2) { $biosInfo.boot_mode = "UEFI" }
    elseif ($firmware -and $firmware.PEFirmwareType -eq 1) { $biosInfo.boot_mode = "Legacy BIOS" }
    
    $bb = Get-CimInstance Win32_BaseBoard -ErrorAction SilentlyContinue
    if ($bb.Manufacturer) { $boardInfo.manufacturer = $bb.Manufacturer.Trim() }
    if ($bb.Product) { $boardInfo.product = $bb.Product.Trim() }
} catch {}
